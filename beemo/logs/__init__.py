...
def is_html(path: str) -> bool:
    p = path.lower()
    return p.endswith("/") or p.endswith(".html") or p.endswith(".htm")
...
def referer_domain(referer_url) -> str:
    if not referer_url or not referer_url.netloc:
        return ""
    domain = referer_url.netloc.lstrip("www.").lower()
...
def process_log_file(gz_file: Path, csv_dir: Path) -> int:
    ...
    with open(csv_path, "w", newline="") as csvf:
        ...
        writer.writerow(["time", "remote_host", "path", "ua", "is_bot", "referer"])
